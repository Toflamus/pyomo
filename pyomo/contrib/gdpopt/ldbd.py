...
class GDP_LDBD_Solver(_GDPoptDiscreteAlgorithm):
    ...
    def _solve_gdp(self, model, config):
        ...
        while True:
            ...
            if info is not None and info.get('source', None) == 'Neighbor':
    ...
    def neighbor_search(self, anchor_point, config):
        ...
        - If the anchor is feasible (objective < `config.infinity_output`),
          evaluate all neighboring points (as generated by the discrete base
          `_generate_neighbors`).
        ...
        for neighbor, _direction in self._generate_neighbors(anchor_point, config):
